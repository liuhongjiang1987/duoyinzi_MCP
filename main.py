...
版本：v0.9.0-MVP
状态：MVP阶段 - 内存模式，功能完整验证通过

功能概述：
- 基于 FastMCP 框架的智能数据分析服务器
- 提供数据上传、字段分析、极性调整、隶属度计算等完整工作流
- MVP阶段采用内存缓存机制，支持会话内数据管理
- 三层架构设计：数据层、算法层、交互层
...
### 保存资源到持久化存储（MVP阶段：仅内存存储，不持久化到文件）
...
def calculate_membership_with_config(resource_id: str, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = {
        ...
        "original_resource_uri": get_resource_uri(resource_id),  # 原始资源完整URI
        "result_resource_uri": result_uri,  # 计算结果完整URI
    ...
    - **原始资源ID**：`{resource_id}`
    - **隶属度计算结果资源ID**：`{result_resource_id}`
...
def perform_topsis_comprehensive_evaluation(resource_id: str = None, weights: List[float] = None, cache_result: bool = True) -> Dict[str, Any]:
    ...
    - **原始资源ID**：`{resource_id}`
    - **TOPSIS计算结果资源ID**：`{result_resource_id}`
...
def _generate_topsis_next_actions(num_objects: int) -> str:
    ...
    return f"""
    **下一步建议：**
    1. **等级综合评定**：可调用 `perform_grade_comprehensive_assessment` 函数进行等级综合评定，获得业务对象的最终等级划分
    2. **结果解读**：相对接近度值越大表示该业务对象在对应评估级别下越接近最优解

    **等级综合评定说明：**
    - **功能**：基于TOPSIS结果的等级综合评定，将相对接近度矩阵转换为明确的等级划分
    - **算法**：计算级别特征值和二元语义，为每个业务对象确定最终等级
    - **输出**：级别特征值、二元语义、最终等级评定结果

    本次评估共分析了 {num_objects} 个业务对象，建议重点关注相对接近度较高的方案，并可进一步进行等级综合评定。
    """
...
def _generate_grade_assessment_next_actions(num_objects: int, num_levels: int) -> str:
    ...
    return f"""
    **下一步建议：**
    1. **等级结果解读**：根据最终等级和符号偏移值分析业务对象的等级分布特征
    2. **等级分布分析**：统计各等级的业务对象数量，分析整体等级分布情况
    3. **符号偏移分析**：分析α值的分布，了解业务对象的等级边界情况
    4. **敏感性分析**：可调整等级数量参数，观察等级划分的稳定性
    5. **对比分析**：可调用 `perform_vikor_comprehensive_evaluation` 函数进行VIKOR对比分析
    6. **数据导出**：可调用 `export_resource_to_csv` 函数导出详细评定结果

    **等级综合评定特点：**
    - **精确等级划分**：将连续的综合隶属度转换为离散的等级结果
    - **符号偏移信息**：提供等级边界的细微差异信息
    - **决策支持**：为资源分配、优先级排序提供量化依据

    本次评定共分析了 {num_objects} 个业务对象，采用 {num_levels} 级评定体系，
    建议重点关注等级分布和符号偏移特征，为后续决策提供支持。
    """
...
def perform_grade_comprehensive_assessment(resource_id: str = None, num_levels: int = 4, cache_result: bool = True) -> Dict[str, Any]:
    """
    执行等级综合评定 - 基于TOPSIS结果的等级综合评定工具
    该函数实现等级综合评定算法，包括级别特征值计算和二元语义评定
    基于公式4-32和4-33实现业务对象的等级综合评定
    
    Args:
        resource_id: TOPSIS结果资源ID（可以是纯ID或完整URI）
                     例如：'mcr_4a291844_769818_topsiseval' 或 'mcr_91f53e05_f85398_topsiseval'
    ...
    Returns:
        ...
        ### 📈 等级综合评定结果
        | 业务对象 | 综合隶属度向量 | 级别特征值 | 二元语义 | 最终等级 |
        |---------|---------------|-----------|----------|----------|
        | T1 | (0.382, 0.297, 0.205, 0.116) | 2.056 | (2, +0.056) | 2级 |
        | T2 | (0.250, 0.250, 0.250, 0.250) | 2.500 | (3, -0.500) | 3级 |
        | T3 | (0.100, 0.200, 0.300, 0.400) | 3.000 | (3, +0.000) | 3级 |
        
        ### 📋 评定说明
        1. **综合隶属度向量**: 归一化后的相对接近度向量，表示业务对象在各等级上的隶属程度
        2. **级别特征值**: 综合隶属度向量的加权平均值，反映业务对象的整体等级倾向
        3. **二元语义**: (k, α) 表示评定等级为k级，符号偏移值为α
        4. **最终等级**: 根据二元语义确定的业务对象等级
        
        ### 🔍 符号偏移值解释
        - **α > 0**: 级别特征值略高于当前等级，偏向更高等级
        - **α = 0**: 级别特征值正好等于当前等级
        - **α < 0**: 级别特征值略低于当前等级，偏向更低等级
        
        ## 🚀 下一步操作建议’‘’
    """
    # 如果没有提供resource_id，尝试自动发现TOPSIS结果资源
    if resource_id is None:
        resource_id = find_latest_topsis_resource()
        if resource_id is None:
            raise ValueError("未找到可用的TOPSIS结果资源（mcr_开头），请先执行TOPSIS综合评估")
        print(f"🤖 自动发现TOPSIS结果资源: {resource_id}")
    ...
    if not _is_topsis_result(topsis_df):
        raise ValueError("数据格式错误：需要TOPSIS结果格式的数据")
    ...
    if cache_result:
        ...
        for obj_name in membership_scores.keys():
            ...
            result_data.append({
                '业务对象': obj_name,
                '综合隶属度向量': str(membership_scores[obj_name]),
                '级别特征值': v_value,
                '二元语义_k': k,
                '二元语义_alpha': alpha,
                '最终等级': f"{k}级"
        ...
        RESOURCE_INDEX[result_resource_id] = {
            ...
            "description": "等级综合评定结果（级别特征值和二元语义）"
        }
        result["result_resource_id"] = result_resource_id
        
        print(f"等级综合评定结果已缓存: {result_uri}")
    ...
    ## 📊 资源信息
    - **原始资源ID**：`{resource_id}`
    - **等级综合评定结果资源ID**：`{result_resource_id}`
...
def _is_topsis_result(df: pd.DataFrame) -> bool:
    """
    检查DataFrame是否为TOPSIS结果格式
    
    Args:
        df: 要检查的DataFrame
        
    Returns:
        如果是TOPSIS结果格式返回True，否则返回False
    """
    if df.empty:
        return False
    
    # 检查是否包含必要的列
    required_columns = ['业务对象', '评估级别', '相对接近度', '综合隶属度']
...
def _extract_membership_scores_from_topsis(topsis_df: pd.DataFrame, num_levels: int) -> Dict[str, List[float]]:
    ...
    Args:
        topsis_df: TOPSIS结果DataFrame
        num_levels: 评估等级数量
        
    Returns:
        综合隶属度向量字典，格式为{"T1": [u1, u2, u3, u4], ...}
    """
    membership_scores = {}
    
    # 按业务对象分组
    grouped = topsis_df.groupby('业务对象')
    
    for obj_name, group in grouped:
        # 按评估级别排序
        group = group.sort_values('评估级别')
        
        # 提取综合隶属度值
        u_values = group['综合隶属度'].tolist()
...
def perform_vikor_comprehensive_evaluation(resource_id: str, weights: List[float] = None, v: float = 0.5, cache_result: bool = True) -> Dict[str, Any]:
    ...
    - **原始资源ID**：`{resource_id}`
    - **VIKOR计算结果资源ID**：`{result_resource_id}`
...
def _generate_vikor_next_actions(num_objects: int, v: float) -> str:
    ...
    return f"""
    **下一步建议：**
    1. **妥协解分析**：分析Q值排序，识别最优妥协解和次优方案
    2. **群体效用分析**：结合S值分析各方案的整体表现
    3. **个体遗憾分析**：结合R值分析各方案的最差属性表现
    4. **等级综合评定**：可调用 `perform_grade_comprehensive_assessment` 函数进行等级综合评定
    5. **敏感性分析**：调整策略权重v（当前v={v}），观察结果稳定性
    6. **对比分析**：可调用 `perform_topsis_comprehensive_evaluation` 函数进行TOPSIS对比分析

    **VIKOR方法特点：**
    - **妥协解概念**：基于群体效用和个体遗憾的平衡
    - **策略可调**：通过v参数调节保守与激进策略
    - **多维度评估**：同时考虑整体表现和个体短板

    本次评估共分析了 {num_objects} 个业务对象，采用策略权重v={v}，
    建议重点关注Q值排序和妥协解特征，为多准则决策提供支持。
    """
